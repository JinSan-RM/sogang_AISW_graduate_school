...
- Final controller follow-up (2026-08-30): the controller started `aisw_p0qa`, set the in-app browser to 320 × 800, opened `/notices`, `/home`, and `/events`, and marked those three tabs as deliverable visible tabs after verifying the same rendered labels and data described above. The controller then stopped only `aisw_p0qa` with its volumes preserved. `C:\Users\yug67\AppData\Local\Temp\pr17-notices-320.png`, `C:\Users\yug67\AppData\Local\Temp\pr17-home-320.png`, and `C:\Users\yug67\AppData\Local\Temp\pr17-events-320.png` are local temporary captures, not durable branch artifacts.
