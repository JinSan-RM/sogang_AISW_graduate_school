...
The admin post-management section separates content by `전체 / 공지사항 / 참여활동 / 커뮤니티·자료 / 원우회` and then by its actual post-backed board. Selecting a board exposes its existing specialized workflow while retaining common open, edit, pin, delete, search, and status controls. Notice, suggestion, mutual-aid, and council-activity boards link to their dedicated admin workflows; organization introductions, FAQ, calendars, guides, and external links remain in their existing dedicated sections rather than appearing as standard post boards. `club-promo` and `networking-programs` cards additionally show the current representative-image thumbnail and allow an administrator to replace the first image attachment in place while preserving the post body, participation URL metadata, deadline, anonymity, and every other attachment. This uses the existing media and post-update APIs and requires no database schema change.
