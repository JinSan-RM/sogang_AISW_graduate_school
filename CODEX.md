...
- Completed QA 175/176 code alignment: deployed media allowlists must include ZIP, text, and Jupyter notebook attachments; production startup now rejects a stale incomplete allowlist. A dry-run-first, explicit-storage-ID repair command corrects the legacy HWP mislabeled as Word and links the seven previously omitted ZIP/TXT/IPYNB files without replacing the live database. Production data repair remains an operator step after coordinated database and media backups.
