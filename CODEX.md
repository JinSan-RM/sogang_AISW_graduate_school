...
- Completed P0 bug #26 alignment: club activity certifications select only published administrator-managed club guide posts, the API validates and canonicalizes that source, and guide renames update existing list/detail tags while retired guides retain their last official historical name. A dry-run-first normalization command audits existing rows and applies only exact or operator-approved mappings; UI structure, styling, and copy remain unchanged.
