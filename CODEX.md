...
- Completed QA 187 home-alumni alignment: Home ends with the approved `동문회 주소록` row and opens only the active `alumni-directory` board's administrator-configured HTTP(S) external link.
- Completed QA 189 notice freshness alignment: selecting `전체`, `학사공지`, `행사공지`, or `기타공지` refetches the board registry and notice posts while preserving the selected filter, so new notices appear without manual pull-to-refresh.
- Completed activity-certification image restoration: certification feeds use the historical fixed `2.05:1` landscape thumbnail and certification detail galleries use the historical full-width `240px` cover frame, while notices, participation guides, and photo albums retain their existing image policies.
