...
- Completed activity-certification image restoration: certification feeds use the historical fixed `2.05:1` landscape thumbnail; club- and study-activity detail galleries use the historical full-width `240px` contain frame, while networking activity keeps the `240px` cover frame. Club-activity feed previews omit the legacy `[동아리명] :` / `[동아리 명] :` template row. Notices, participation guides, photo albums, and other activity text remain unchanged.
