...
- Completed functional bugs #3 and #4: the notice feed now combines every active notice board, excludes the calendar board, and applies academic/event/webinar/other filtering consistently so the all/other tabs no longer omit valid posts. Home uses the same active notice-board set and shows the two newest deduplicated notices without pin priority.
