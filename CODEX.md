...
- Completed QA 180 admin-content alignment: post management is organized by content-group and actual-board tabs with board-specific descriptions and links to existing dedicated workflows. Club and networking guide cards expose their current representative image and replace the first image attachment directly while preserving all post fields and remaining attachments; the implementation reuses existing APIs and adds no database schema or migration work.
