...
- Completed P0 account-deletion alignment: authenticated deletion requires the current password; the public email request/verify path is non-enumerating; user PII, sessions, private/draft/hidden/mutual-aid content and private media are irreversibly deleted; public published posts/comments and required public attachments are retained only after author/owner unlinking and filename anonymization. Migration `0021` records a non-identifying receipt without user ID, email, IP address, or deletion counts.
...
- Completed functional bugs #3 and #4: the notice feed now combines every active notice board, excludes the calendar board, and applies academic/event/webinar/other filtering consistently so the all/other tabs no longer omit valid posts.
...
- Completed resource-post edit alignment: authors and administrators can move an existing resource-sharing post among the active resource boards; the API enforces target permissions and preserves the post's related content.
...
- Implemented in code: one mutual-aid read policy covers list/detail/search/comments/reactions/media/reports and returns object-hiding `404` for non-owners.
