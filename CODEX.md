...
- Completed QA 161 refresh alignment: Home, Notices, and shared board lists use pull-to-refresh while retaining their cached content and current filters/search/scroll state; paginated post feeds refresh only page 1, keep the previous rows until success, and replace the first cached page without discarding later pages. Protected media URLs remain stable until an image load failure explicitly refreshes them, and notification delivery/bootstrap refresh remains independent.
...
- Completed board-feed pagination alignment (2026-08-30): Notices, Community photo albums, Resource `전체`, individual resource boards, Participation guides/certifications, and Council activity history use 20-row incremental pages with guarded next-page loading. Notice/Resource `전체`/Council activity use one permission-filtered aggregate API query instead of per-board fan-out; Home performs one bounded latest-notices request with `size=2`. Header Back retains the mounted filter/list state, while explicit Notices, Community, and Participation bottom-tab presses keep the existing reset to `전체`, `행사 사진첩`, and `동아리 > 안내`.
- Verified board-feed pagination on 2026-08-30: backend compile and full pytest passed (`331 passed, 1 skipped`, one third-party deprecation warning); frontend `npm test` passed (`390 passed`), typecheck/export passed, and lint had `0` errors with `9` unrelated pre-existing warnings. The isolated Docker QA stack was healthy, and mobile-sized Chrome flow checks observed exactly one page-2 request after scrolling for Notices, Community photo albums, Resource `전체`, lecture reviews, Participation guides, and activity certifications; filter requests restarted at page 1, detail header Back retained state, and tab reselection restored the existing defaults. Native pull gestures still require iOS/Android device or emulator QA, and the QA database had zero Council activity rows, so its empty page-1 state was verified visually while its multi-page behavior remains covered by API/hook tests rather than this browser dataset.
- Completed activity-certification image restoration: certification feeds use the historical fixed `2.05:1` landscape thumbnail, while every activity-certification detail gallery uses the available full width and each image's natural aspect-ratio height without a fixed `240px` frame or collapsed preview. Club-activity feed previews omit the legacy `[동아리명] :` / `[동아리 명] :` template row. Notices, participation guides, photo albums, and other activity text remain unchanged.
- Completed QA 180 unified admin-board alignment: the single `게시판 관리` entry gives 공지사항, 커뮤니티·자료, 참여활동, and 원우회 the same group → actual board → 콘텐츠/운영 설정 structure. An administrator can change the club representative image at `참여활동 → 동아리 홍보 → 콘텐츠`; club and networking guide cards replace only the first image attachment while preserving every post field, metadata value, and remaining attachment. The implementation reuses existing APIs and requires no database schema change, Alembic migration, or manual DB work.
